#!/usr/bin/env python3
"""Leave-One-Subset-Out Ridge 回帰 (DeepGaitV2 特徴量のみ、SUSTech1K版・共通モデル抽出)。

lodo_ridge_dgv2.py(per-subset自前学習版)との違いは入力JSONのみ:
- 入力: dgv2_analysis/metrics_dgv2_sustech.json
  (特徴抽出器 = CASIA-Bを学習していない共通のSUSTech1K版DeepGaitV2。
   train-on-train を解消した版。認識器/targetはper-subset版と同じ)
- 特徴量: log_people + DeepGaitV2 系5指標のみ (Inception/DINO/生画素は使わない)
- 目的変数: acc_nm, acc_bg, acc_cl, acc_all
- 評価: MAE, RMSE, Spearman, Ridge 係数平均

実行: docker exec opengait_container python /app/OpenGait/dgv2_analysis/lodo_ridge_sustech.py
"""
import json
import os
...
JSON_PATH = os.path.join(HERE, "metrics_dgv2_sustech.json")
...
def main():
    ...
    with open(os.path.join(HERE, "result_ridge_sustech.json"), "w") as f:
        json.dump(out, f, indent=2, ensure_ascii=False)
    print(f"\nwrote {os.path.join(HERE, 'result_ridge_sustech.json')}")
